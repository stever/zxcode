...
# INCBIN resolve relative to program.asm. Names are relative paths (e.g.
# lib/util.asm) staged under their folders — mirroring the project's download
...
def stage_project_files(workdir, files):
    ...
    for pf in files or []:
        ...
        # or its outputs just like a file, so the rules cover every segment.
        if any(seg.split('.', 1)[0] == 'program' for seg in segments):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Path '{pf.name}' is reserved for the main source")
        # A staged *.tap/*.nex — file or directory — would be picked up by
        # the output scan below and break the exactly-one-output rule.
        if any(seg.endswith(('.tap', '.nex')) for seg in segments):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Path '{pf.name}' clashes with compiler output; "
