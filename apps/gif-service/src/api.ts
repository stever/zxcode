...
// gif-service talks to the GraphQL api as the unauthenticated `public` role:
// that role can read public projects (its select permission filters to
// is_public = true) and call the compile / compileC actions. No admin secret
// is sent, so the service keeps holding no secrets.
const GRAPHQL_URL = process.env.GRAPHQL_URL ?? 'http://api:8080/v1/graphql';
// Cap every api call so a hung upstream compiler (zxbasic/z88dk) or a slow
// lookup can't tie up a request indefinitely.
const GRAPHQL_TIMEOUT_MS = parseInt(process.env.GRAPHQL_TIMEOUT_MS ?? '20000', 10);
...
async function gql<T>(query: string, variables: Record<string, unknown>): Promise<T> {
    ...
    try {
        ...
        if (controller.signal.aborted) {
            throw new Error(`api request timed out after ${GRAPHQL_TIMEOUT_MS}ms`);
        }
        throw err;
    } finally {
        clearTimeout(timer);
...
/**
 * Compile through one of the api's compile mutations (Boriel ZX BASIC, z88dk
