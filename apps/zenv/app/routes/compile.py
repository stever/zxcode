...
class CompileResult(BaseModel):
    base64_encoded: str
    # Kept for parity with the CompileResult the Hasura action type declares;
    # zenv compiles Forth at RUNTIME (the outer interpreter), so there is no
    # build-time line->address map to return.
...
def generate_user_asm(code: str) -> str:
    ...
    and blank lines are dropped. Text is emitted as DB byte values only,
    never as quoted strings, so no user input can escape into assembler
    directives.
    """
    ...
    out = [
        ...
        "; evaluated line by line at boot (see zenv-boot.patch).",
        "user_boot:",
        "\tCALL colon_code",
    ]
    for i, data in enumerate(lines):
        out.append(f"\tDX literal_raw: DW user_line_{i}: "
                   f"DX literal_raw: DW {len(data)}: DX evaluate")
    out.append("\tDX exit")
    for i, data in enumerate(lines):
        out.append(f"user_line_{i}:")
