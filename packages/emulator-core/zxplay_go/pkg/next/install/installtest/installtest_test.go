...
func TestIsUnderTempDir_MacOSPaths(t *testing.T) {
	cases := []string{
		...
		"/var/folders/x/y/T/something/zxplay_go/next",
	}
	for _, c := range cases {
		if !isUnderTempDir(c) {
...
func TestIsUnderTempDir_RealPathsRejected(t *testing.T) {
	cases := []string{
		"/Users/dev/Library/Application Support/zxplay_go/next", // macOS real config
		"/home/user/.config/zxplay_go/next",                     // Linux real config
		`C:\Users\user\AppData\Roaming\zxplay_go\next`,          // Windows real config
		"/etc/foo",
		"/var/lib/something",
		"",
	}
