...
import (
	...
	rtcpkg "github.com/stever/zxplay_go/pkg/next/rtc"
...
// uses (cmd/zxplay_go/next.go dmaPortBus).
...
func newNext() (*Harness, error) {
	...
	// (cmd/zxplay_go/next.go: spec-faithful timing.md §1c model). Without it
	...
	// Same as the production wiring (cmd/zxplay_go/next.go): the classic
	...
	// cmd/zxplay_go/next.go: IO endpoints route through the ULA's port
	...
	// (cmd/zxplay_go/next.go): enabled chain sources / NMIs pause transfers.
	...
	// Fast-path enablement (#187) — mirrors cmd/zxplay_go/next.go.
