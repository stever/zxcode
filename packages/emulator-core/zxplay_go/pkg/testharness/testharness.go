...
func New(model roms.SpectrumModel) (*Harness, error) {
	...
	u := ula.New(mem, kbd)
	cpu := z80.New(mem, u)
	// Per-access ULA memory contention, same as cmd/zxplay_go's
	...
	// pacing loop, #194).
	cpu.MemContend = true
	// Beam-time paper capture, as in cmd/zxplay_go's newEmulator (#194).
	...
	// Fast-path enablement (#187) — mirrors cmd/zxplay_go/main.go.
