...
What runs in zxplay_go. This is a working document — every entry has a
...
zxplay_go yet; foundation tests confirm the 48K boot path works.
...
| Exolon | Works | #192. Hewson custom loader: BASIC + $FC00 code trap-load, then LD-EDGE-trapped edge-timed loading (enters the ROM loop directly, so the $0556 block trap never sees those blocks). Loads in a few seconds in the browser. Corpus row locks the title screen. |
| Firelord | Works | #192. Hewson custom loader: one 49152-byte headerless block over the whole RAM via a RAM byte loop calling ROM LD-EDGE (LD-EDGE-trapped). Loads in a few seconds in the browser. Corpus row locks the joystick menu. |
...
A general note rather than specific titles: zxplay_go's mid-frame border
...
part of zxplay_go, and the most likely place to hit a bug.** `.nex`
...
| Space Invaders (Next) | Works (gameplay) | FIXED r94 (#204): "doesn't progress past initial screen" was a COMPOSITOR gap, not a game hang — the arcade-faithful port draws its entire overlay (score header, invader field, INSERT COIN, the in-game action) on the classic ULA screen with NR$14=$00 (black transparent) layered ABOVE a 320×256 hi-res Layer 2 planet backdrop via NR$15 priority USL, and the wide-L2 render path never repainted classic ULA pixels over the L2 overlay (the long-documented U-above-wide-L2 residue), so only the planet showed while the game ran perfectly underneath — attract cycling, coin (K) and start (1) all worked invisibly. Sounds stream via zxnDMA burst transfers to the $DF DAC port (counter-poll paced). Fixed by the CaptureULABase pure-ULA snapshot + composeULAOverlayRow repaint in OverpaintWideL2Row (see known-gaps wide-frame row; pinned by TestULAAboveWideL2USL + the ZX_GO_SI_PROBE=1 end-to-end probe: overlay visible, coin+start enter game mode). Note the game is an arcade cabinet clone: it wants K (coin) then 1 (start), per its "PUSH +/K FOR CREDITS" attract text. |
...
| Bomb Jack (Next) | Works (gameplay) | FIXED r98 (#202): "joystick has reverse controls" — the game reads its in-game controls ONLY as keyboard rows via configurable row/mask pairs ($6885 left/$6887 right/$6889 up/$688B down/$688D jump); its "2 - JOYSTICK" menu option sets the Sinclair 1 layout (6/7/8/9/0 on row $EFFE), and its single Kempston read is a menu wait-for-fire. Our pad never reached keyrows (the FPGA's NR$05 membrane key-joystick injection wasn't modelled), the frontend "Sinclair 1" label injected keys 1-5 (naming swapped vs the Next's official nextreg.txt naming — Sinclair 1 IS 67890), and the reported "reversed/overlapping" behaviour was the Cursor scheme's 5/6/7/8 landing on the game's 6/7/8/9 reads (up→right, down→left, right→down — authentic Cursor-vs-Sinclair rotation, on real hardware too). Fixed by the membrane_stick model + NR$05-driven frontend routing + the naming swap (see known-gaps NR$B0-$B2 row). Pinned end-to-end by TestBombJack202Probe (ZX_GO_BJ_PROBE=1, local game folder + Browser launch): menu key 2 → pad fire starts the game through the injection → pad right/left moves Jack the commanded way ($43DA X 128→216→84). Round-1 gameplay renders correctly (Sphinx backdrop, platforms, bombs, HUD). |
...
| Way of the Exploding Fist (Next) | Works (gameplay) | FIXED by #165 — TWO stacked bugs. (1) The staging tool's FAT32 `appendDirent` extended a full directory without zeroing the new cluster; on the distro image's dirty free space, stale bytes interleaved with new entries and detached `Palettes/HighscoreTable.npl`'s LFN chain from its 8.3 entry, so the game's F_OPEN failed — and the game's own error path then uploaded a stale palette buffer from `$B000+1` (an esxDOS error CARRY rippling through its `RL E` address math). (2) With staging fixed, every post-keypress screen rendered in four shades of dark blue: our NR$44 half-pair latch wasn't reset by NR$40/$41/$43 writes (zxnext.vhd:5376/5382/5395) — the game's clear routine leaves a dangling half-pair and relies on the reset. #206 ("may need 60 Hz to work properly"): NOT a frequency problem — the game reads NR$05 bit 2 at .nex start and picks its `music/50/` or `music/60/` assets, and the headless probe (`TestWOTEF206Probe`, ZX_GO_WOTEF_PROBE=1) runs intro → title → menu → attract → live fights on BOTH refresh paths (60 Hz simulated by poking NR$05 bit 2 post-boot; geometry retunes to 60192 T/264 lines per #182). The real browser symptom was performance: the game's ~23 kHz CTC/hw-IM2 music-mix ISR (NR$C0=$09, CTC ch2, 1.5 DAC samples/scanline on $DF) is fat enough that the #208 gate's ISR-body per-instruction chain polling cost ~30% of exec — 43 fps, audio pulled ~38k/44.1k, continuous underruns from the title on. Fixed by the #206 updateActive refinement (see known-gaps hw-IM2 row); verified in the play app at r95: title music streams at full 44.1k with the underrun counter flat (one transient burst at the intro→title decompress), intro/title/menu render correctly via the game-zip route. |
...
zxplay_go and a reference emulator can boot: stage the game's files plus
...
zxplay_go and ZEsarUX in seconds (ZEsarUX reached Baggers' `.nex` entry
...
`TestCompatibilityCorpus` (in `cmd/zxplay_go`) turns this manifest into an
...
ZX_GO_CORPUS=/path/to/games go test ./cmd/zxplay_go/ -run TestCompatibilityCorpus -v

# Record/refresh a title's golden screen hash (new title, or intended
# rendering change)
ZX_GO_CORPUS_UPDATE=1 ZX_GO_CORPUS=/path/to/games \
  go test ./cmd/zxplay_go/ -run TestCompatibilityCorpus -v
```
