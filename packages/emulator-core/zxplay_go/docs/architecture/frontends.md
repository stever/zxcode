...
Build: `GOOS=js GOARCH=wasm go build ./cmd/zxplay_go` via
...
- Assets: the PRIMARY source is currently the staged `/next/` assets
  ...
  absent (`cmd/zxplay_go/distro_prep.go`); staged/user images mount
...
`remoteDebugger` (`cmd/zxplay_go/debugger.go`) is the shared backend.
