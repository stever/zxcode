...
FPGA core (TBBlue). zxplay_go therefore emulates the Next by treating the
...
4. Divergence hunting against a reference emulator uses the dev modes in
   `cmd/zxplay_go` (`--next-lockstep`, `--next-nrdiff`, `--next-memdiff`,
...
- Direct-core boot (`ZX_GO_NO_FPGA_BOOTROM=1` +
  ...
  `cmd/zxplay_go/next_directboot.go`. The seed table was captured from a
  live boot and is coupled to the SD distro version; the re-verification
  procedure lives in `packages/emulator-core/README.md`.
- Fastboot (`cmd/zxplay_go/fastboot.go`, `zxFastBoot()` export): pure time
...
- Copper NR$02 NMI delivery (#187): the render passes run AFTER the
  ...
  The render RegWriter therefore FILTERS NR$02 (cmd/zxplay_go
  ...
  `ExtNMIFunc` poll (cmd/zxplay_go copperNMIPacer — live during HALT,
  ...
  wired from the CPU reference clock in cmd/zxplay_go) that Step,
...
- SD card (`pkg/next/sdcard`): a protocol-accurate SPI-mode card:
  ...
  boot — `cmd/zxplay_go/distro_prep.go` deletes the first-boot welcome
