...
Module path: `github.com/stever/zxplay_go`. Roughly 145k lines of Go
...
- Desktop: `go build ./cmd/zxplay_go` (cgo: Fyne needs a C toolchain and
  OpenGL). `Makefile` adds convenience targets and version stamping.
- wasm: `scripts/build-wasm.sh` in the parent package runs
  `GOOS=js GOARCH=wasm go build ./cmd/zxplay_go` → `dist/zx.wasm` plus
