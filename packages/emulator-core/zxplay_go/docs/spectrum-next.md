# ZX Spectrum Next support in zxplay_go

zxplay_go supports the ZX Spectrum Next as `ModelNext` — a separate machine
...
1. Download the official Spectrum Next distribution zip from
   `https://www.specnext.com/distro/` (the 24.11 release is the one zxplay_go
...
There is one boot path now: the **authentic cold boot**. `./bin/zxplay_go
...
zxplay_go does not redistribute any Spectrum Next ROM. The classic Sinclair
