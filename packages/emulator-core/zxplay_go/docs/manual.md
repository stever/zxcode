...
This is the everyday user guide to **zxplay_go** — how to start it, pick a machine,
...
documented separately (run `zxplay_go --help`, and see
...
- **[COMPARISON.md](../COMPARISON.md)** — how zxplay_go compares to other emulators
...
Or skip the flags and hand it a file — `zxplay_go game.tap` picks the machine
...
window**, or by handing the file to zxplay_go at launch (`zxplay_go game.tap`, or a
file-manager double-click — see
[Launching a file directly](#launching-a-file-directly-command-line--double-click)).
zxplay_go picks the right loader from the extension.
...
zxplay_go does proper **tape emulation** (the signal is decoded by the ROM loader,
...
`zxplay_go FILE` boots straight into a program: the extension picks the machine
...
- **An explicit model flag beats the extension** — `zxplay_go --pentagon game.tap`
  boots the Pentagon with the tape mounted.
- **Flags come before the file**: `zxplay_go --headless --frames=2500 game.nex`,
...
type above, so double-clicking a Spectrum file opens it in zxplay_go. No root
...
- **CRT scanline filter** — dims one screen row per emulated scanline for
  the retro look. The frame is scaled straight onto the display's pixel
  grid, so the gaps stay evenly spaced at every zoom. Needs at least 2x
  (200%, or a window at least twice the frame's height) for a gap to have
  a row to live in; below that the picture is drawn without scanlines.
...
- **Help → About zxplay_go** — version and credits.
...
For anything deeper — timing, hardware conformance, the debugger, or how zxplay_go
