...
func newDebuggerCore(emu *emulator, pauseAtStart bool, historySize int, historyWide bool) *remoteDebugger {
	...
	emu.cpu.BreakpointCheck = func(pc uint16) bool {
		...
		// Line-call breakpoints (compiled BASIC, linecallbp_cmd.go):
		// one atomic load + compare when disarmed.
		if d.checkLineCallBP(pc) {
			return true
		}
		// Source-line step for address-map languages (stepline_cmd.go):
		// one atomic load when disarmed.
		if d.checkStepLine(pc) {
			return true
		}
...
func (d *remoteDebugger) connLoop(c net.Conn) {
	...
	w := bufio.NewWriter(c)
	_, _ = w.WriteString("OK welcome to zxplay_go remote debugger\r\n")
	_ = w.Flush()
...
func (d *remoteDebugger) handleCommand(line string) string {
	...
	case "linecall-anchor":
		return d.cmdLineCallAnchor(args)
	case "set-linecall-bp":
		return d.cmdSetLineCallBP(args)
	case "clear-linecall-bp":
		return d.cmdClearLineCallBP(args)
	case "list-linecall-bps":
		return d.cmdListLineCallBPs()
	case "linecall-step":
		return d.cmdLineCallStep(args)
	case "step-line-anchors":
		return d.cmdStepLineAnchors(args)
	case "step-line":
		return d.cmdStepLine(args)
