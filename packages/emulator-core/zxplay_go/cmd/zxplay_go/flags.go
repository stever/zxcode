...
type cliFlags struct {
	...
	// boot straight into (`zxplay_go game.tap`). applyLaunchFile derives the
...
func parseCLI() *cliFlags {
	...
	flag.Parse()
	if *versionFlag {
		fmt.Printf("zxplay_go %s (built %s)\n", zxlog.Version, version.Date())
		os.Exit(0)
	}
	if *exportIcon != "" {
	...
	// (`zxplay_go game.tap`) — the file-manager / double-click contract.
