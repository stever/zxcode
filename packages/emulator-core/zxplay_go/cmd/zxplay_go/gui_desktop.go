...
import (
	...
	"github.com/stever/zxplay_go/pkg/config"
	...
	"github.com/stever/zxplay_go/pkg/zxlog"
)
...
func desktopMain() {
	...
	a := app.NewWithID("org.zxplay.zxplaygo")
	...
	// Positional launch file (`zxplay_go game.tap`, file-manager double-click):
	...
	mainMenu := fyne.NewMainMenu(
		...
		fyne.NewMenu("Help",
			fyne.NewMenuItem("About zxplay_go", func() {
				dialog.ShowInformation("About zxplay_go",
					version.String()+"\n"+
						"built "+version.Date()+"\n\n"+
...
func (e *emulator) confirmImportNex(fileName string, data []byte) {
	...
	msg := fmt.Sprintf(
		"To load %q the way a real Spectrum Next does — through NextZXOS's own loader, so games that depend on the operating system work correctly — a copy must be written to your SD card (as /zx.nex at the card root).\n\n"+
