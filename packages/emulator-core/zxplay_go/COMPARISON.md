...
This document compares **zxplay_go** with the three most prominent ZX Spectrum Next
emulators: **CSpect**, **ZEsarUX**, and **MAME** (its `tbblue` driver).

It is written and maintained by the zxplay_go project, but the goal is a *fair* and
...
- **Feature presence ≠ maturity.** A green check means "implemented," not "as
  ...
  MAME is the community's accuracy/preservation reference. **zxplay_go is young**
...
| Criterion | zxplay_go | CSpect | ZEsarUX | MAME |
|---|:---:|:---:|:---:|:---:|
| **Open source** | ✅ | ❌ | ✅ | ✅ |
...
ZEsarUX covers a very wide range of 8-bit/16-bit home computers. zxplay_go and
...
⁷ zxplay_go runs the genuine ZX80 (4K) and ZX81 (8K) ROMs with a faithful
...
documented TODO. zxplay_go implements collision and the max-per-line status flag;
the per-line *bandwidth* render limit is visual-only and deferred.
⁸ Copper in zxplay_go is driven from a per-T-state beam-position model but rendered
per-scanline (full per-pixel hpos deferred). CSpect/ZEsarUX Copper timing is
documented as sub-scanline-imprecise.
⁹ ULANext palette handling in zxplay_go is wired for the static-palette paths the OS
...
¹¹ zxplay_go targets cycle accuracy with contention that scales by turbo multiplier;
...
¹⁴ zxplay_go's boot path was verified divergence-by-divergence against the FPGA
...
¹⁷ zxplay_go can mount a host directory (building a FAT image on the fly) *or* a
pre-built `.img`/`.mmc`.
...
²² zxplay_go supports RZX playback **and** recording, including competition-mode DSA
...
²³ zxplay_go persists battery-backed RTC NVRAM to disk.
²⁴ zxplay_go implements the zxnDMA's memory↔memory and memory↔IO transfers
...
²⁵ zxplay_go provides a UART/AT-command stub; real Wi-Fi networking is out of scope.
...
²⁷ zxplay_go has two surfaces — a live GUI debugger and a scriptable telnet server —
...
natively.
²⁹ zxplay_go offers a snapshot ring with rewind (not continuous reverse execution).
ZEsarUX's "time machine" is the most complete reverse-debugging implementation
here.
³⁰ zxplay_go exposes a custom line-oriented telnet protocol; it is not DeZog/DZRP/
ZRCP-compatible.
...
³² zxplay_go has no plugin API, but offers extensive headless trace/automation flags.

---

## 10. Maturity & positioning

| | zxplay_go | CSpect | ZEsarUX | MAME |
|---|---|---|---|---|
...
**Where zxplay_go stands out today:** it is a clean, fully open-source (MIT
...
ZEsarUX (and verify on real hardware). zxplay_go's aim is to be a *correct, readable,
