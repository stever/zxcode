...
| No exhaustive I/O-PORT decode conformance test (the NextReg register decode is covered by TestNRDecodeConformance above; the port-address decode — $xx3B blocks, Kempston, DAC aliases, internal-port-enable gating — still has no single enumerating test) | audit gap (matrix axis 4, #158) |
