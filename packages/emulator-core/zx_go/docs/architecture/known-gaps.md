...
| Gap | Status | Source |
| --- | --- | --- |
| Port $0B (the legacy Zilog-DMA port — the zxnDMA's "Z80 DMA compatibility mode" decode, specnext wiki DMA page) is unclaimed: only $6B is wired, so writes to $0B vanish and reads return $FF. Found by a manual run of Misc/DmaInteractive, whose DEFAULT port is $0B (caps+P switches it to $6B) | deferred | `pkg/ula/ula.go` (port dispatch), `pkg/next/dma/dma.go` |
