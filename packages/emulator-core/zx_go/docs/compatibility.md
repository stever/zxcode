...
| TX-1696 | Known issue | #169 pinned the r49 residual to the LINE: the game's audio-install runs a raster-phased stack blast — poll NR$1F ≥ 192 ($B168: `LD A,$1F`/NR read/`CP $C0`), ~18 lines of setup, then CALL $EDE8 = a 4,632-byte `PUSH HL` slide (SP from $4CFE down through the $2000-$3FFF ROM window, writes past $4000 discarded on purpose). The frame INT (cvc 248, zxula_timing.vhd c_int_v=1) must land mid-slide with SP in the window: the discarded interrupt pushes make the IM1 exit's final `RET` (ROM3 $0050 path) pop a ROM3 data word, PC lands in the game's `$E5` flood covering high RAM, surfs the pushes to a collector, and the game resumes — verified end-to-end in-emulator by staging SP/PC/beam at the designed geometry (the #166 FW `JP (IX)` hook-dispatch theory is retired: bank1 $2009 does drivers+keyboard+FRAMES only). Two real conformance gaps found and FIXED en route (r50): the 28 MHz wait state applied to M1 fetches only instead of EVERY memory read (zxnext.vhd:3168-3181 — opcode, prefix, operand AND data reads wait; specnext wiki confirms NOP=5T), and the CTC block not being wired at all (ports $183B-$1F3B, NR$C5, pulse-mode INT — the game's install hook arms CTC ch0 at 250 kHz as its IM2 heartbeat; zxnext.vhd:4064-4093/2014-2043). REMAINING BLOCKER (open): with every constant measured and VHDL-verified (entry cvc ~210.5, slide 30.5 lines at 12T/push, wrap-1 at 241 hits ROM3 $0000 = `DI`, INT at 248.3 arrives ~1.3 lines after the second wrap, dead every retry, ~3.98M-insn retry period), the design only closes if real-hardware 28 MHz `PUSH` costs ≥ 15.1T (i.e. writes ALSO wait, which neither the vendored 3.02.03 VHDL nor community docs show) or the INT fires ≥ 7.3 lines earlier. The raster poll re-locks the phase every retry so no upstream jitter (SD Nac variance was added — faithful in itself — but is absorbed by the poll). HARDWARE-MEASURED (2026-07-15, `_tools/hw-probe` on a real Next): NOPs 33 lines/12k (5T exact = our model), pushes 40 lines/6k (~12.2T — writes do NOT wait; ROM-window writes = RAM writes), frame INT at raster 248 (exact). Instruction timing is exonerated on silicon — the install geometry cannot close on real hardware either as measured. The OS-version hypothesis is DEAD (owner, 2026-07-15): his real Next runs NextZXOS 2.09 (the same 24.11-era distro our card uses) and plays the game, and his working copy is byte-for-byte the tested 66 KB Downloads build (a 197 KB `main.nex` found on his card is the game's DEMO — a red herring that briefly mis-closed #169). LAUNCH PATH TESTED AND EXONERATED (2026-07-15): the owner's flow — Browser → the game's own `run-tx-1696.bas` (REG 7,3, 50 Hz check, LAYER 1,2 → LAYER 1,1: CLS, then in-folder `.nexload main.nex`) — was replicated headless on a 256 MB BuildFAT32 card with the game at the owner's real path `/games/Next/TX-1696/` (Browser driven by `--press-key` cursor chords, `caps+6`/`caps+7`). The game launches, loads further than ever reported (draws its animated starfield), then spins the SAME audio-install raster-poll retry loop forever: PC histogram dominated by $B168-$B16F/$8AD4-$8ADE (the `LD A,$1F`/NR-read/`CP $C0` poll), input-deaf, screen state periodic in frames. So autoexec-vs-launcher makes no difference; the launcher preamble does not shift the slide-entry phase into the INT catch window. CARD IMAGE THEN ELIMINATED TOO (2026-07-15, reverse arbitration): the forward dd test could never run — no image we build boots real firmware 1.44 (see the BuildFAT32 work item) — so the owner's own 29 GB card dd (`next-working.img`) was booted directly in the emulator instead (new `sdcard.FileSource` streams large images with a RAM write-overlay). His real working copy lives at `/home/games/Next/TX-1696/` (byte-identical to the Downloads build; the `/games/Next/TX-1696/` copy is the 197 KB demo that misled an earlier session), and driving his exact Browser → launcher flow on his exact image wedges IDENTICALLY: starfield, then HALT at PC=$4CF7 (SP=$4CF0, IM=2, IFF1=false) — the same terminal PC as every staged-card run — input-deaf, screen frame-periodic (twinkle likely copper-driven). With bytes, card, launch path, OS (2.09) and core (his 3.02.00 works) all held equal and the game playing on his silicon, the divergence had to be an emulator gap — and a real-hardware debug session (2026-07-16, dezogif + the `hwdebug/dzrp.py` DZRP client over the owner's joy-port serial cable) MEASURED IT: breakpoints on the real Next showed the slide call RETURNING INLINE (bp $AE0B hit, SP restored to $4D00), and the paused state revealed the actual catch mechanism — the install masks the ULA frame INT entirely (NR$22=$06) and arms **CTC ch0 in the Next's hardware-IM2 vectored mode** (NR$C0=$AD bit 0, NR$C5=$01, IM2 table at $5Exx → handler $8001). A CTC interrupt lands within microseconds of slide entry — no raster geometry involved; the whole frame-INT-at-248 analysis targeted an interrupt the game had switched off. ROOT CAUSE: the emulator wires the CTC's legacy pulse INT (r50) but not the NR$C0 bit-0 hardware-IM2 vectored path (im2_control.vhd/im2_peripheral.vhd — level assert, hardware-supplied vector at IM2 acknowledge), so nothing catches the slide and PC runs off its end. Fix pending (Axis 5). The owner's card dd (`ZX_GO_NEXT_SD_IMG=next-working.img`, streamed via `FileSource`) + the recorded Browser key schedule reproduces the failure deterministically for the fix work. Loader is also FAT-geometry sensitive (512 B-cluster cards abort earlier; BuildFAT32 2 KB clusters get furthest). |
