...
| zilogDMA.snx | Misc/ZilogDMA | The same transfer matrix programmed the Zilog way through port $0B: length N moves N+1 bytes (the dma_mode -1 counter seed), fixed destinations LOADed with the direction flipped, CONTINUE short-inits, both hex read-back lines and the 4T/byte flashing-border timing bands — pinned to the core-3.1.5 board photo. Drove wiring port $0B, the LOAD-latched src/dst pointer model and the harness Next's narrow frame-INT pulse (vendored 2026-07-13) |
